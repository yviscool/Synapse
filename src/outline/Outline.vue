<template>
  <!--
    ============================================================================
    主容器 (The Master Container) - v3
    ----------------------------------------------------------------------------
    引入 "创生" 理念，初始状态优雅，动画哲学重塑为 "绽放"。
    ============================================================================
  -->
  <div ref="containerRef" :style="containerStyle" :class="[
    ...
    <div :class="[
      ...
      <div class="flex items-center gap-3 overflow-hidden">
        <span
          :class="[ICONS.title, 'text-xl text-blue-500 dark:text-blue-400 transition-transform duration-300 flex-shrink-0', isCollapsed && isContainerHovered ? 'scale-110 -rotate-12' : '', isCollapsed ? 'text-2xl' : '']"></span>
        ...
        <div v-show="!isCollapsed" class="flex items-center gap-1">
          <div class="relative group/btn">
            <button @click.stop="handleRefresh" class="control-btn" aria-label="Refresh">
              <span
                :class="[ICONS.refresh, 'text-lg transition-transform duration-700', isRefreshing ? 'animate-spin-fast' : '']"></span>
            </button>
    ...
    <Transition name="content-bloom">
      <div v-show="!isCollapsed" class="flex-1 flex flex-col min-h-0 overflow-hidden bg-transparent">
        ...
        <div class="px-4 py-3 flex-shrink-0">
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none"><span
                :class="[ICONS.search, 'text-gray-400 text-sm']"></span></div>
        ...
        <div class="flex-1 overflow-y-auto px-3 pb-3 custom-scrollbar" ref="listContainerRef">
          ...
          <div v-if="isLoading" class="pt-4 space-y-3">
            <div v-for="i in 5" :key="i" class="h-8 bg-gray-200/50 dark:bg-gray-700/50 rounded-lg animate-pulse"></div>
          </div>
          ...
          <TransitionGroup v-else name="list-item" tag="ul" class="space-y-1" :key="listKey">
            <li v-for="(item, index) in filteredItems" :key="item.id" :data-index="index"
              class="outline-item-wrapper group" :class="{ 'is-active': index === highlightedIndex }"
              @click="(e) => handleItemClick(e, item.element)" @mousemove="(e) => handleItemHover(e, index)"
              @mouseleave="handleItemLeave" @mouseenter="handleItemMouseEnter(item.element)">
              <div class="outline-item-content">
                <span class="item-index">{{ index + 1 }}</span>
                <span class="item-icon transition-all duration-200"
                  :class="[getDisplayIcon(item, index), hoveredItem.index === index ? 'text-blue-500 scale-110' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-300']"></span>
...
import { useDraggable, until, useWindowSize } from "@vueuse/core";
...
// --- 状态管理 (新增 isInitializing, listKey, isMounted) ---
...
// ★ 刷新逻辑：引入 listKey 来触发 Vue 的销毁/重建，从而播放完整的列表动画
...
const stats = computed(() => {
  ...
  if (isLoading.value) return "Loading..."; // ★ 使用 isLoading
...
function handleItemMouseEnter(element: Element) {
  document
    .querySelectorAll(".outline-highlight-target")
    .forEach((el) => el.classList.remove("outline-highlight-target"));
  element.classList.add("outline-highlight-target");
}
...
const ElegantTooltip = defineComponent({
  props: { text: String },
  setup(props) {
    return () => h(Transition, { name: 'tooltip-fade' }, {
      default: () => h('div', {
        class: [
...
.outline-highlight-target::after {
  content: '';
  position: absolute;
  inset: -4px;
